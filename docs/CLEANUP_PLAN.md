...
- End-to-end pipeline test (simulator → detectors → backend)
- Kafka message flow validation
- FL round completion test
...
**Sample Code**:
- `examples/custom_detector.py` - How to add new anomaly detector
- `examples/kafka_consumer.py` - Consume events from Kafka
- `examples/fl_client_custom.py` - Custom FL client with different model

#### 2.4 Add Python Packaging
...
name = "fedics"
version = "0.1.0"
description = "Federated Intrusion Detection for Critical Systems"
...
### Phase 3: Security & Operations (High Priority for Production)
